...
export default function ChangeDetectionPanel({ beforeScene, afterScene, onAfterUploaded, result, onResult }) {
  ...
  async function handleCompare() {
    ...
    setRunning(true)
    try {
      const comparison = await runChangeDetection(beforeScene.scene_id, afterScene.scene_id)
      onResult(comparison)
    } catch (err) {
      setError(err.message)
    } finally {
  ...
  return (
    <div className="change-panel">
      ...
      <button type="button" className="secondary-action" disabled={!beforeScene?.accepted || uploading} onClick={() => inputRef.current?.click()}>
        {uploading ? 'Uploading after scene…' : afterScene ? 'Replace after scene' : 'Upload after-date GeoTIFF'}
      </button>
      <button type="button" className="compare-button" disabled={!beforeScene?.accepted || !afterScene?.accepted || running} onClick={handleCompare}>
        {running ? 'Comparing…' : 'Compare Changes'}
      </button>
      ...
      {result && (
        <div className="change-results">
          ...
          <div className="transition-list">
            {result.transitions.length === 0 ? <p>No classified transitions detected.</p> : result.transitions.slice(0, 4).map((item) => (
              <div className="transition-item" key={`${item.from_class}-${item.to_class}`}><span>{item.from_class}</span><b>→</b><span>{item.to_class}</span><em>{item.percent_of_changed}%</em></div>
            ))}
          </div>
